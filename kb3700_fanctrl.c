...
static void dump_fan_config(void)
{
	...
	for (i = 0; i < 8; i++)
	{
		printf("%3d ", read_ec(EC_TEMP_THRESHOLDS_2 + i));
	}
	putchar('\n');
	printf("Fan speeds (%%):       ");
	for (i = 0; i < 8; i++)
	{
		printf("%3d ", read_ec(EC_FAN_SPEEDS_2 + i));
	}
	putchar('\n');
}
...
static void reset_speeds(void)
{
	...
	{
		write_ec(EC_FAN_SPEEDS_2 + i, defaults[i]);
	}
}
			...
			if (strcmp(argv[0], "-f") == 0)
			{
				...
				if (argc > 0)
				{
					...
					while (argc > i)
					{
						...
						write_ec(EC_FAN_SPEEDS_2 + i, atoi(argv[0]));
						argv++;
						i++;
					}
